...
void LRE3Texture::Load(std::string filePath)
{
    ...
    {
        data = LRE3Texture::LoadXPM(filePath, &x, &y, &n);
        // return;
    }
    else
...
void LRE3Texture::Delete()
{
    glDeleteTextures(1, &m_texture);
}

GLuint LRE3Texture::GetTextureID() const
{
    return m_texture;
}

void LRE3Texture::SetName(std::string name)
{
    this->m_name = name;
}
std::string LRE3Texture::GetName() const
{
    return this->m_name;
}

unsigned char* LRE3Texture::LoadXPM(std::string xpmStr, int* x, int* y, int* n)
{
